...
**MCP server** — 24 tools over Streamable HTTP at `/mcp` (or the stdio bridge for clients that can't speak remote MCP), covering full CRUD, partial edits with optimistic concurrency, keyword/semantic/hybrid search, history, restoration, attachment management, and runtime control of semantic search itself. See [docs/mcp.md](docs/mcp.md).
