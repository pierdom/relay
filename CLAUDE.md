...
**Per-client consent gate** (relay #313 Stopgap): `RelayOAuthProvider.authorize()` only forwards straight to PocketID for a DCR client an `oauth.db`-persisted human approval already covers (`OAuthStore.is_client_approved`/`approve_client`) — otherwise it routes to relay's own unauthenticated `GET/POST /mcp/oauth/consent` (`relay/mcp_oauth/consent.py`) first. This closes a confused-deputy gap: PocketID skips its own consent screen for a client the user has already approved once, and relay registers exactly one static client with PocketID, so *every* DCR-registered relay client — attacker's or legitimate — looked pre-approved to PocketID regardless of whether a human had ever seen it. The consent page sets a `__Host-`/`Secure`/`HttpOnly`/`SameSite=Lax` cookie (signed `itsdangerous`, distinct salt from the web-UI session) binding the browser that viewed the prompt (`GET`) to the one that submits Approve/Deny (`POST`) — a captured consent link opened in a different browser fails that check.

**Approval is granted in `broker.handle_callback`, not in consent's own `POST` handler** — a security-audit finding on the first version of this feature (relay #313, caught before merge): DCR registration and `/authorize` are both unauthenticated by spec, so an attacker can register their own client and click through their own consent page with zero relay credentials. If that click granted approval directly, the attacker could self-approve their own client in advance, then send a victim a link that silently rides the pre-approval straight through to PocketID — reintroducing the exact bypass this whole feature exists to close. Consent's `POST` only forwards to PocketID; the actual grant happens only after `broker.handle_callback` validates a real PocketID id_token *and* that `sub` clears the allowlist, so an attacker's self-click dead-ends at a login screen they can't pass. Approval is per `client_id`, persists across restarts, and — once granted — is permanent (no separate revoke UI yet; wipe `oauth.db` per the Runbook in relay post #313 to reset). Expected to be superseded by fastmcp's own `ConsentMixin` when the fastmcp migration (same post) lands; not removed until then.
