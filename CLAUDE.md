...
uv run pytest -q                # 350 tests (incl. 73 browser smokes)
...
- **Six themes: Relay Dark (default), Relay Light, ANSI Dark, ANSI Light, Gruvbox, Tokyo Night.** The ANSI pair is the IBM VGA 16-colour set (the `#aa`/`#55` one a terminal shows, not Microsoft's `#800000`/`#c0c0c0` variant) and it forced two decisions worth keeping: with nothing between `#aaaaaa` and `#ffffff`, **cards are framed rather than filled** — `--surface` equals `--bg` and the border does the work, because inventing an off-black would have broken the palette rule — and **there is no lightness hierarchy in text**, prose and headings both sitting at 21:1 with emphasis carried by colour, exactly as a terminal does it. Its accent is bright cyan, not green: green is already `--green`, and an accent indistinguishable from the connected dot is a bad trade for phosphor nostalgia. **Picker order is a rule, not a list** — the two signature themes lead, in that order, then everything else alphabetically by label; `theme.js` sorts rather than maintaining a sequence, so theme #5 lands in the right place by existing. Tokyo Night is the first *cool* palette and was added specifically as the harder test: the literal check catches a wrong *value*, never a token used for the wrong *role*, and every theme before it was amber or yellow. It needed no change outside its own block, and `tests/ui/test_themes.py` derives its theme list from the stylesheet so a new theme is covered without a test edit. A theme is *one token block and one registry entry in `js/theme.js`* — gruvbox and tokyo-night each needed nothing else, which is what the token refactor was for. Palette values come from the upstream theme (gruvbox from relay #104 and `relay_tui/palettes/gruvbox.toml`; tokyo-night from folke/tokyonight.nvim, using storm's `bg` as the elevated surface and `fg_gutter`/`dark3` as the two border steps) rather than being mixed by hand. Gruvbox's values are not invented: they are the corellia desktop identity (relay post #104), and `relay_tui/palettes/gruvbox.toml` anchors on the same three, so desktop, terminal, TUI and browser agree. **`dark` is the bare `:root` block and is represented by *no* attribute**, so an absent or unrecognised stored value degrades to a complete theme; it is *also* named via `:root, [data-theme="dark"]` so it can be selected like the others. Theme blocks match a bare attribute selector as well as `:root` — that is what lets the picker's swatches be live samples (`<span class="theme-swatch" data-theme="gruvbox">` is painted by gruvbox's own tokens) instead of hex values restated in JS.
